...
from app.services.bus import BusService, parse_departures
...
def test_parse_sorts_and_drops_departed():
    data = parse_departures(SAMPLE, BusConfig(), NOW)
...
def test_line_filter_and_name_override():
    cfg = BusConfig(line_filter=["31"], stop_name="Hjemme")
    data = parse_departures(SAMPLE, cfg, NOW)
...
async def test_quay_id_uses_quay_query():
    ...
    assert data.stop_id == "NSR:Quay:9"
