...
from app.routers import bus, lights, music, system, weather
...
from app.services.sonos import SimSonosService, SonosService
...
def create_app(config: AppConfig) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        # Musikk: Spotify (spillelister, ev. avspilling) og Sonos (lokal avspilling)
        spotify_svc = sonos_svc = None
        if config.spotify.enabled:
            if config.spotify.simulate:
                spotify_svc = SimSpotifyService(config.spotify)
            else:
                spotify_svc = SpotifyService(config.spotify, app.state.http,
                                             config.resolve(config.spotify.token_file))
        if config.sonos.enabled:
            sonos_svc = SimSonosService(config.sonos) if config.sonos.simulate else SonosService(config.sonos)
        app.state.music = MusicService(sonos=sonos_svc, spotify=spotify_svc)
